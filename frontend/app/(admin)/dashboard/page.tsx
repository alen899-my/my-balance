    ...
    <div className="w-full bg-card border border-border rounded-none flex flex-col shadow-sm relative overflow-hidden group h-[480px]">
      ...
      <div className="p-4 border-b border-border/50 flex items-center justify-between bg-muted/10 shrink-0">
         <div className="flex items-center gap-3">
            <div className={cn("w-8 h-8 flex items-center justify-center rounded-none border-2 rotate-45 group-hover:rotate-0 transition-all duration-500", type === "debit" ? "bg-destructive/10 text-destructive border-destructive/20" : "bg-emerald-500/10 text-emerald-600 border-emerald-500/20")}>
               <div className="-rotate-45 group-hover:rotate-0 transition-transform duration-500">{type === "debit" ? <TrendingDown className="w-4 h-4" /> : <TrendingUp className="w-4 h-4" />}</div>
            </div>
            <div>
               <h3 className="text-[10px] font-black uppercase tracking-[0.2em] leading-none">{title}</h3>
            </div>
         </div>
      ...
      <div className="flex-1 overflow-x-hidden overflow-y-auto custom-scrollbar">
         <table className="w-full text-left border-collapse table-fixed">
            <thead className="sticky top-0 z-40 bg-card shadow-sm">
               <tr className="bg-muted/5 border-b border-border/40">
                  <th className="px-5 py-3 text-[9px] font-black text-muted-foreground/60 uppercase tracking-[0.15em] w-[55%]">Description</th>
                  <th className="px-3 py-3 text-[9px] font-black text-muted-foreground/60 uppercase tracking-[0.15em] text-center w-[15%]">Count</th>
                  <th className="px-5 py-3 text-[9px] font-black text-muted-foreground/60 uppercase tracking-[0.15em] text-right w-[30%]">Volume</th>
               </tr>
            </thead>
            <tbody>
               {data.length === 0 ? (
                  <tr><td colSpan={3} className="py-20 text-center text-[10px] uppercase font-black text-muted-foreground/20 px-5 tracking-[0.4em]">No Data</td></tr>
               ) : (
                  data.map((item, idx) => {
                     const isTop3 = idx < 3;
                     const medalColors = ["text-amber-400", "text-slate-300", "text-orange-400"];
                     return (
                        <tr key={idx} className={cn("border-b border-border/20 last:border-0 transition-all duration-300 group/row cursor-default", isTop3 ? "bg-muted/10 font-bold" : "hover:bg-muted/30")}>
                           <td className="px-5 py-3">
                              <div className="flex items-start gap-3">
                                 <div className="w-5 flex justify-center mt-0.5">
                                    {isTop3 ? <Medal className={cn("w-4 h-4", medalColors[idx])} /> : <span className="text-[10px] font-black text-muted-foreground/15 italic">#{(idx+1).toString().padStart(2, '0')}</span>}
                                 </div>
                                 <div className="flex flex-col min-w-0 flex-1">
                                    <div className={cn("text-[12px] font-bold leading-tight truncate", isTop3 ? "text-foreground" : "text-foreground/70 group-hover/row:text-primary")}>{item._id || "Unknown"}</div>
                                 </div>
                              </div>
                           </td>
                           ...
                           <td className={cn("px-5 py-3 text-right text-[13px] font-black tabular-nums", type === "debit" ? "text-destructive" : "text-emerald-600")}>{currencySymbol}{item.amount.toLocaleString()}</td>
                        </tr>
                     );
                  })
               )}
      ...
      <div className="shrink-0 bg-muted/20 border-t border-border/60 px-5 py-3 flex justify-between items-center text-[9px] font-black uppercase tracking-widest text-muted-foreground/50">
        <span>TOTAL VOLUME</span>
        <span className={cn("text-[14px]", type === "debit" ? "text-destructive" : "text-emerald-600")}>{currencySymbol}{totalAmount.toLocaleString()}</span>
      </div>
    </div>
  );
}
...
function RecentTransactionsList({ transactions, currencySymbol, loading, title, subtitle, colorScheme = "primary" }: {
  ...
  title: string,
  subtitle: string,
  colorScheme?: "primary" | "destructive" | "emerald"
}) {
  ...
  return (
    <div className="w-full h-[400px] bg-card border border-border flex flex-col shadow-sm relative overflow-hidden group">
      ...
      <div className="p-4 border-b border-border/40 flex items-center justify-between bg-muted/5 shrink-0">
        <div><h3 className={cn("text-[10px] font-black uppercase tracking-[0.15em]", textClasses[colorScheme])}>{title}</h3><p className="text-[8px] font-bold text-muted-foreground/40 uppercase mt-0.5">{subtitle}</p></div>
      </div>
      <div className="flex-1 p-1 space-y-0.5 overflow-y-auto custom-scrollbar">
        {transactions.map((tx: any, idx: number) => {
            const isDebit = (tx.debit || 0) > 0;
            return (
              <div key={tx._id || idx} className="flex items-center justify-between p-2 rounded-none hover:bg-muted/40 transition-all border border-transparent hover:border-border/10">
                <div className="flex items-center gap-3 overflow-hidden">
                  <div className={cn("shrink-0 w-8 h-8 flex items-center justify-center border", isDebit ? "bg-destructive/5 text-destructive border-destructive/10" : "bg-emerald-500/5 text-emerald-600 border-emerald-500/10")}>{isDebit ? <ArrowUp className="w-4 h-4" /> : <ArrowDown className="w-4 h-4" />}</div>
                  <div className="overflow-hidden">
                    <div className="text-[11px] font-black leading-tight whitespace-normal break-words">{tx.description || tx.payee}</div>
                    <div className="text-[8px] font-bold text-muted-foreground/40 uppercase mt-1">{tx.date} • {tx.bank}</div>
                  </div>
                </div>
                <div className={cn("shrink-0 text-[12px] font-black tabular-nums tracking-tighter pl-2", isDebit ? "text-destructive" : "text-emerald-600")}>{currencySymbol}{(tx.debit || tx.credit).toLocaleString()}</div>
              </div>
            );
          })}
      </div>
...
export default function DashboardPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    if (dashboardFetchCache[cacheKey] && (now - dashboardFetchCache[cacheKey].timestamp < CACHE_TTL)) {
       const cached = dashboardFetchCache[cacheKey].data;
       setTransactionsData(cached.summary); setMonthlyTrend(cached.monthly_trend); setTopDebits(cached.top_debits); setTopCredits(cached.top_credits); setRecentDebit(cached.recent_debit); setRecentCredit(cached.recent_credit); setBanks(cached.banks); setRecordStats(cached.records); setLoading(false); return;
    }
    ...
    try {
      const [banksRes, insightsRes, debitRes, creditRes] = await Promise.all([
        ...
        fetch(`${API_BASE_URL}/insights?${baseParams.toString()}`, { headers: { Authorization: `Bearer ${token}` } }),
        fetch(`${API_BASE_URL}/transactions?limit=15&type=debit&${baseParams.toString()}`, { headers: { Authorization: `Bearer ${token}` } }),
        fetch(`${API_BASE_URL}/transactions?limit=15&type=credit&${baseParams.toString()}`, { headers: { Authorization: `Bearer ${token}` } })
      ]);
      
      const [banksJson, insightsJson, debitsJson, creditsJson] = await Promise.all([
        banksRes.json(), insightsRes.json(), debitRes.json(), creditRes.json()
      ]);
  ...
  return (
    <AdminPageLayout
      ...
      onClearFilters={() => { setBankFilter("All Banks"); setSearchFilter(""); }}
    >
      <div className="flex flex-col gap-4 w-full pb-6">
         ...
         <div className="grid grid-cols-1 lg:grid-cols-6 gap-4 w-full">
            ...
            <div className="lg:col-span-2 grid grid-cols-1 gap-4">
               <RecentTransactionsList title="Recent Spent" subtitle="Latest 15 Debits" transactions={recentDebit} currencySymbol={currencySymbol} loading={loading} colorScheme="destructive" />

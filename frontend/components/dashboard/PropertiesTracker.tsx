...
import { Plus, MoreHorizontal, Edit3, Trash2, Eye, Image as ImageIcon, MapPin, Building, Home, UploadCloud, X, Camera, ArrowUpRight } from "lucide-react";
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const payload = {
        type,
        title: title.trim(),
        cent_area: centArea ? parseFloat(centArea) : null,
    ...
    <Modal open={open} onClose={onClose} title={editEntry ? "Edit Property" : "Add Property"} size="md">
      <div className="flex flex-col gap-4 py-1">
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <div 
            className={cn(
              "relative flex flex-col items-center justify-center w-full h-40 border-2 border-dashed rounded-xl overflow-hidden cursor-pointer transition-colors",
              previewUrl ? "border-primary/50 bg-background" : "border-border hover:bg-muted/40"
            )}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <FormInput label="Asset Type" value={type} onChange={e => setType(e.target.value)} placeholder="e.g. Plot, Laptop, Gadget" />
          <FormInput label="Title/Name" value={title} onChange={e => setTitle(e.target.value)} placeholder="e.g. Munnar Plot" />
        </div>
    ...
    <Modal open={open} onClose={onClose} title="Property Details" size="sm">
      <div className="flex flex-col gap-4 py-2">
        {entry.image_url ? (
          ...
          <div className="w-full h-32 border rounded-lg bg-muted flex items-center justify-center text-muted-foreground/50">
...
export function PropertiesTracker() {
  ...
  const getIcon = (type: string) => {
    ...
    if (lowerType.includes("land") || lowerType.includes("plot")) return <MapPin className="w-5 h-5 text-amber-500" />;
    if (lowerType.includes("house") || lowerType.includes("villa")) return <Home className="w-5 h-5 text-sky-500" />;
    if (lowerType.includes("building")) return <Building className="w-5 h-5 text-indigo-500" />;
    ...
    <>
      ...
      <div className="flex flex-col gap-6">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="w-full sm:w-48 z-20 relative">
            <Select
              value={activeTab}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {filteredEntries.map(prop => (
              <div key={prop._id} className="group flex flex-col bg-card rounded-2xl border border-border shadow-sm hover:shadow-md hover:border-border/80 transition-all relative overflow-hidden">
                ...
                {/* Image hero */}
                <div className="h-44 relative bg-muted flex items-center justify-center overflow-hidden">
                  {prop.image_url ? (
                    <img src={prop.image_url} alt={prop.title} className="w-full h-full object-cover transition-transform duration-700 ease-in-out group-hover:scale-105" />
                  ) : (

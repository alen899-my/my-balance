...
export function StatementUploadModal({ open, onClose, onSuccess }: StatementUploadModalProps) {
  ...
  const handleReset = () => {
    setFile(null); setBank(""); setCustomBank(""); setPassword("");
    setError(null); setSuccessMsg(null); setPollStatus(null);
  };
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (selected.type !== "application/pdf") {
      setError("Please upload a valid PDF file."); setFile(null); return;
    }
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data.status === "completed") {
          ...
          setError(data.message || "Processing failed."); setUploading(false); setPollStatus(null);
        } else {
          setPollStatus(`Processing… (${data.processed_pages}/${data.total_pages} pages)`);
          setTimeout(() => checkStatus(jobId, token), 2000);
        }
      } else {
        setError("Failed to check status."); setUploading(false);
      }
    } catch {
      setError("Status check error."); setUploading(false);
    }
  };

  const handleUpload = async () => {
    if (!file) { setError("Please select a statement PDF file."); return; }
    const finalBank = bank === "Other..." ? customBank.trim() : bank;
    if (!finalBank || finalBank === "Select a bank") { setError("Please specify the bank name."); return; }

    setUploading(true); setError(null); setSuccessMsg(null); setPollStatus("Uploading file…");
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) throw new Error(data.detail || "Failed to upload statement.");

      if (data.jobId) {
        setPollStatus("Upload complete. Processing…");
        checkStatus(data.jobId, token || "");
      } else {
        setUploading(false); setSuccessMsg("Statement uploaded successfully."); onSuccess?.();
      }
    } catch (err: any) {
      setError(err.message || "An error occurred during upload.");
      setUploading(false); setPollStatus(null);
    }
  };

  const uploadDisabled =
    !file || !bank || (bank === "Other..." && !customBank.trim()) || uploading;
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-4 py-2 min-h-[440px]">

        {/* Error banner — only shown when NOT uploading */}
        {error && !uploading && (
        ...
        {/* ── Storytelling animation while uploading / after success ── */}
        {uploading || successMsg ? (
          <ScanningAnimation
            status={successMsg ? "Complete" : pollStatus}
            error={error}
          />
        ) : (
          <>
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <Select
                ...
                onChange={(val) => { setBank(val); if (val !== "Other...") setCustomBank(""); }}
              />
              {bank === "Other..." && (
              ...
              <div className={cn(
                ...
                {file ? (
                  ...
                  <div className="flex flex-col items-center gap-1.5">
                    ...
                    <p className="text-sm font-medium text-foreground text-center">Click to drop your statement</p>
                    <p className="text-xs text-muted-foreground text-center">PDF format only. Max 10 MB.</p>
                  </div>
                )}
              </div>
            </div>
